...
import type {
  ...
  PluginHookAfterLlmResponseEvent,
...
export function createHookRunner(registry: PluginRegistry, options: HookRunnerOptions = {}) {
   ...
   * Allows plugins to synchronously observe the exact input payload before the LLM call.
   */
  async function runBeforeLlmRequest(
    event: PluginHookBeforeLlmRequestEvent,
    ctx: PluginHookAgentContext,
  ): Promise<void> {
    return runVoidHook("before_llm_request", event, ctx);
  }
  ...
  /**
   * Run after_llm_response hook.
   * Allows plugins to synchronously observe the exact output payload before the run continues.
   */
  async function runAfterLlmResponse(
    event: PluginHookAfterLlmResponseEvent,
    ctx: PluginHookAgentContext,
  ): Promise<void> {
    return runVoidHook("after_llm_response", event, ctx);
  }
