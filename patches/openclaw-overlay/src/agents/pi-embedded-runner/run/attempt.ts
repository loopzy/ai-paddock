...
export async function runEmbeddedAttempt(
  ...
  log.info(
    `[paddock-attempt] start runId=${params.runId} sessionId=${params.sessionId} workspace=${resolvedWorkspace}`,
  );
  ...
  try {
    ...
    log.info(
      `[paddock-attempt] skills-ready runId=${params.runId} sessionId=${params.sessionId} fromSnapshot=${params.skillsSnapshot ? "yes" : "no"} entries=${skillEntries?.length ?? 0}`,
    );
    ...
    log.info(
      `[paddock-attempt] bootstrap-context-ready runId=${params.runId} sessionId=${params.sessionId} bootstrapFiles=${hookAdjustedBootstrapFiles.length} contextFiles=${contextFiles.length}`,
    );
    ...
    log.info(
      `[paddock-attempt] tools-created runId=${params.runId} sessionId=${params.sessionId} disableTools=${params.disableTools ? "yes" : "no"} toolsRaw=${toolsRaw.length}`,
    );
    ...
    log.info(
      `[paddock-attempt] system-prompt-ready runId=${params.runId} sessionId=${params.sessionId} promptChars=${appendPrompt.length} tools=${tools.length}`,
    );
    ...
    log.info(
      `[paddock-attempt] acquiring-session-lock runId=${params.runId} sessionId=${params.sessionId} sessionFile=${params.sessionFile}`,
    );
    ...
    log.info(
      `[paddock-attempt] session-lock-acquired runId=${params.runId} sessionId=${params.sessionId}`,
    );
    ...
    try {
      ...
      log.info(
        `[paddock-attempt] session-manager-ready runId=${params.runId} sessionId=${params.sessionId} hadSessionFile=${hadSessionFile ? "yes" : "no"}`,
      );
      ...
      if (extensionFactories.length > 0) {
        ...
        log.info(
          `[paddock-attempt] resource-loader-ready runId=${params.runId} sessionId=${params.sessionId} extensionFactories=${extensionFactories.length}`,
        );
      }
      ...
      log.info(
        `[paddock-attempt] creating-agent-session runId=${params.runId} sessionId=${params.sessionId} builtInTools=${builtInTools.length} customTools=${allCustomTools.length}`,
      );
      ...
      log.info(
        `[paddock-attempt] agent-session-ready runId=${params.runId} sessionId=${params.sessionId}`,
      );
      ...
      try {
        ...
        try {
          ...
          if (hookRunner?.hasHooks("before_llm_request")) {
            try {
              await hookRunner.runBeforeLlmRequest(
                {
                  runId: params.runId,
                  sessionId: params.sessionId,
                  provider: params.provider,
              ...
              log.warn(`before_llm_request hook failed: ${String(err)}`);
            }
          }
      ...
      if (hookRunner?.hasHooks("after_llm_response")) {
        try {
          await hookRunner.runAfterLlmResponse(
            {
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
          ...
          log.warn(`after_llm_response hook failed: ${String(err)}`);
        }
      }
